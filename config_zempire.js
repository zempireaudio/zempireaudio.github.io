...
var init_phones = ["ZE51B Silicone","Bad Guy 2022"],                             // Optional. Which graphs to display on initial load. Note: Share URLs will override this set
      ...
      watermark_text2 = "ZEMPIREAUDIO",
      watermark_image_url = "img/ZE.png",   // Optional. If image file is in same directory as config, can be just the filename
      page_title = "ZE",                     // Optional. Appended to the page title if share URLs are enabled
...
function watermark(svg) {
    ...
    if ( watermark_text ) {
        wm.append("text")
            .attrs({x:622, y:300, "font-size":16, "text-anchor":"middle", "class":"graph-name"})
            .text(watermark_text);
        wm.append("text")
            .attrs({x:642, y:320, "font-size":16, "text-anchor":"middle", "class":"graph-name"})
...
d3.json('config_zempire.json').then(function(mycfg){
